...
export type ChatRequestMessageRole =
  (typeof CHAT_REQUEST_MESSAGE_ROLES)[number];
 ...
 * @param options.minLength 允许的最小长度。
...
/**
 * 校验可选字符串并返回去除首尾空白后的结果。
 * @param value 待处理的值。
 * @param options 配置项。
 * @param options.fieldName 字段名称。
 * @param options.minLength 允许的最小长度。
 * @param options.maxLength 允许的最大长度。
 * @returns 返回校验通过后的字符串；空值时返回 `undefined`。
 */
 ...
 * @param options.max 允许的最大值。
 * @param options.defaultValue 未传值时使用的默认值。
 * @returns 返回校验通过后的整数值。
 */
